...
export const crearEstado = async (req, res) => {
    ...
    try {
        ...
        const existingEstado = await prisma.estadoPlanDeNegocio.findFirst({
            where: {
        ...
        // Crear la servicio
        const nuevoEstado = await prisma.estadoPlanDeNegocio.create({
            data: {
                estado: nombre_estado,
...
export const listarEstados = async (req, res) => {
    try {
        const estados = await prisma.estadoPlanDeNegocio.findMany();
...
export const actualizarEstado = async (req, res) => {
    ...
    const data = req.body;
    try {
        const updatedEstado = await prisma.estadoPlanDeNegocio.update({
            where: {
                id: Number(id),
            },
            data: {
...
export const eliminarEstado = async (req, res) => {
    const { id } = req.params;
    try {
        const findEstado = await prisma.estadoPlanDeNegocio.findUnique({
            where: {
                id: Number(id),
            },
        });
        ...
        await prisma.estadoPlanDeNegocio.delete({
            where: {
                id: Number(id),
            },
        });
