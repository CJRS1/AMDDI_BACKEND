...
export const crearEstado = async (req, res) => {
    ...
    try {
        ...
        const existingEstado = await prisma.estadoArticulo.findFirst({
            where: {
        ...
        // Crear la servicio
        const nuevoEstado = await prisma.estadoArticulo.create({
            data: {
                estado: nombre_estado,
...
export const listarEstados = async (req, res) => {
    try {
        const estados = await prisma.estadoArticulo.findMany();
...
export const actualizarEstado = async (req, res) => {
    ...
    const data = req.body;
    try {
        const updatedEstado = await prisma.estadoArticulo.update({
            where: {
                id: Number(id),
            },
            data: {
...
export const eliminarEstado = async (req, res) => {
    const { id } = req.params;
    try {
        const findEstado = await prisma.estadoArticulo.findUnique({
            where: {
                id: Number(id),
            },
        });
        ...
        await prisma.estadoArticulo.delete({
            where: {
                id: Number(id),
            },
        });
