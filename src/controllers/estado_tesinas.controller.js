...
export const crearEstado = async (req, res) => {
    ...
    try {
        ...
        const existingEstado = await prisma.estadoTesinas.findFirst({
            where: {
        ...
        // Crear la servicio
        const nuevoEstado = await prisma.estadoTesinas.create({
            data: {
                estado: nombre_estado,
...
export const listarEstados = async (req, res) => {
    try {
        const estados = await prisma.estadoTesinas.findMany();
...
export const actualizarEstado = async (req, res) => {
    ...
    const data = req.body;
    try {
        const updatedEstado = await prisma.estadoTesinas.update({
            where: {
                id: Number(id),
            },
            data: {
...
export const eliminarEstado = async (req, res) => {
    const { id } = req.params;
    try {
        const findEstado = await prisma.estadoTesinas.findUnique({
            where: {
                id: Number(id),
            },
        });
        ...
        await prisma.estadoTesinas.delete({
            where: {
                id: Number(id),
            },
        });
